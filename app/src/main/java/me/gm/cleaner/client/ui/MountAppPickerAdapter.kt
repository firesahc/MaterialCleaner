...
class MountAppPickerAdapter(private val fragment: Fragment) :
    BaseKtListAdapter<AppListModel, MountAppPickerAdapter.ViewHolder>(CALLBACK) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        binding.icon.setImageDrawable(
            model.packageInfo.applicationInfo.loadIcon(fragment.requireContext().packageManager)
        )
        ...
        binding.summary.text = run {
            ...
            if (model.readOnlyCount > 0) {
                summary += fragment.getString(
                    R.string.enabled_read_only_count, model.readOnlyCount
                )
            }
        ...
        binding.root.setOnClickListener {
            ...
            if (navController.currentDestination?.id == R.id.mount_app_picker_fragment) {
                val direction = MountAppPickerFragmentDirections
